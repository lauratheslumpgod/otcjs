UI.AddLabel("            Skeet style logs");
UI.AddCheckbox( "Write logs in chat" );
...
function hitlog() {
    ...
    if (me == attackerIndex && me != victimIndex) {
		Cheat.PrintColor([250, 166, 24, 255], "[onetap] ");
    if (hittype == "Hit ") {
        if (UI.GetValue("Script items", "Write logs in chat")) {
            Cheat.PrintChat(" \x08[\x0conetap\x08] [\x0c"+shots.toString()+"\x08] "+hittype+name+"'s \x10"+HitgroupName(hitbox)+"\x08 for \x07"+target_damage.toString()+"\x08 ("+target_health.toString()+" health remaining), aimed \x10"+hitboxName+"\x08 ("+predicthc.toString()+"%), safety=\x03"+safety+"\x08 (\x10"+flags+"\x08) (\x10"+simtime+"\x08:\x10"+exploit+"\x08)\n");
        }
      Cheat.PrintColor([255, 255, 255, 255], "Shot ID: "+shots.toString()+" | "+hittype+name+"'s "+HitgroupName(hitbox)+" for "+target_damage.toString()+" ("+target_health.toString()+" health remaining) | Aimed at the "+hitboxName+" ("+predicthc.toString()+"%%) | Safety = "+safety+" ("+flags+") ("+simtime+":"+exploit+")\n");
  		logs.push(" "+hittype+name+"'s "+HitgroupName(hitbox)+" for "+target_damage.toString()+" ("+target_health.toString()+" health remaining) | Aimed at the "+hitboxName+" ("+predicthc.toString()+"%) | Safety = "+safety+" ("+flags+") ("+simtime+":"+exploit+")");
    }
    else {
      Cheat.PrintColor([255, 255, 255, 255], "Shot ID: "+shots.toString()+" | "+hittype+name+"'s "+HitgroupName(hitbox)+" for "+target_damage.toString()+" ("+target_health.toString()+" remaining) \n");
...
function onDraw() {
    ...
    var font = Render.AddFont("Consolas", 10, 0);
    ...
    if (shots_fired > hits && (Globals.Curtime() - lastUpdate > 0.33)) {
      ...
      if (!logged) {
        ...
        Cheat.PrintColor([250, 166, 24, 255], "[onetap] ");
        Cheat.PrintColor([255, 255, 255, 255], "Shot ID: "+shots.toString()+" | "+"Missed "+Entity.GetName(target)+"'s "+hitboxName+ " ("+predicthc.toString()+"%%) due to "+reason+" | Safety = "+issafe+" ("+flags+") ("+simtime+":"+exploit+")\n");
